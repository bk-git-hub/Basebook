...
## Auth and Admin Extensibility

현재 MVP에는 회원가입과 로그인을 넣지 않는다.

대신 구조는 아래 원칙으로 잡는다.

### 1. 모든 사용자 소유 데이터는 owner 기반으로 설계한다

- `DiaryEntry`는 `ownerId`를 가진다
- `SeasonBookProject`도 `ownerId`를 가진다
- MVP에서는 고정 `demo user`로 동작해도 괜찮다

### 2. 인증은 뒤에 붙여도 도메인 구조를 다시 짜지 않게 한다

- 일반 사용자 엔드포인트는 그대로 유지한다
- 추후 세션 확인용 `/auth/session`을 추가할 수 있게 한다
- 컨트롤러 레벨에서 `current actor`를 주입하는 구조를 염두에 둔다

### 3. 운영자 기능은 별도 namespace로 분리한다

- 운영자 조회는 `/ops/*` 아래로 둔다
- 일반 사용자용 엔드포인트와 섞지 않는다

### 4. MVP에서는 ops-lite까지만 가능하다

정식 관리자 콘솔은 넣지 않지만, 추후 아래 정도는 쉽게 붙일 수 있어야 한다.

- 시즌북 프로젝트 목록 조회
- orderUid와 bookUid 확인
- 주문 상태 확인

### Current Recommendation

- MVP: 로그인 없음, 단일 데모 사용자 기반
- 구조: ownerId / role / auth namespace / ops namespace 선반영
- 확장: 나중에 auth provider를 붙여도 API 도메인과 화면 구조를 크게 갈아엎지 않음
...
6. 추후 인증과 운영자 기능을 Nest 계층에 자연스럽게 추가할 수 있다
