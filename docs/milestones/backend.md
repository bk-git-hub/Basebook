...
- Date: `2026-04-14`
- Time: `20:49`
- Milestone: Public sample-image hosting prepared through Cloudflare R2
- Status: `ready_for_review`
- Summary:
  - `apps/api` now includes a batch upload script for `apps/web/public/sample-images`
  - the script successfully uploaded 18 sample images to public R2 URLs
  - public asset responses were verified with `200 OK` after upload
- Related Decision IDs:
  - `API-025`
- Blocking Items:
  - frontend still needs to swap any local sample-image references to the new public URLs when ready
- Next Step:
  - hand off the public asset mapping to frontend or update backend-owned demo references if the user chooses that path later
