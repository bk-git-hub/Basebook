...
| Browser Smoke / Full-Stack | `src/e2e/entry-create.smoke.spec.ts`, `src/e2e/entry-create.local-full-stack.spec.ts` | 실제 Chromium과 production build에서 라우트와 로컬 API 연동이 뜨는지 본다 | 최소 라우트 진입, 모바일 제어, 로컬 업로드 기반 생성 흐름 | 주문/상태/실패 케이스 전체는 보지 않는다 |
...
- `/entries/new`의 실제 업로드 -> 생성 -> 상세 진입 흐름이 로컬 풀스택 기준으로 이어진다.
