...
| Browser Smoke | `src/e2e/entry-create.smoke.spec.ts` | 실제 Chromium과 production build에서 라우트가 뜨는지 본다 | 최소 라우트 진입과 핵심 셸 렌더링 | 실제 생성/수정/업로드 흐름 전체는 보지 않는다 |
| Scripted Integration | `scripts/order-flow-report.cjs` | 프론트-백 happy path를 실제로 검증하고 증거를 남긴다 | 업로드, 견적, 주문 완료 흐름이 이어진다 | 예외 케이스와 전체 회귀를 대신하지 않는다 |
...
- 최소한 핵심 라우트 하나는 real browser + production build 기준으로 정상 진입된다.
- Next build/start 이후 화면 셸이 즉시 깨지는 종류의 문제를 빨리 잡을 수 있다.

### `test:e2e:order-report`가 통과하면 말할 수 있는 것

- 로컬 환경에서 프론트와 백엔드가 실제로 이어져 happy path를 수행했다.
- 업로드 -> 견적 생성 -> 주문 접수 -> 완료 화면까지 끊기지 않았다.
