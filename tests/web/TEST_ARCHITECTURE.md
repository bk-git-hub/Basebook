...
- 브라우저에서 일부러 만들기 어려운 비정상 API payload 전체를 다 자동화했다고 말할 수는 없다.
- 외부 Sweetbook sandbox 시간 경과 상태까지 완전히 자동화됐다고 말할 수는 없다.

## 현재 알려진 테스트 공백

- 브라우저에서 만들기 어려운 estimate/order 비정상 payload 케이스
- 외부 Sweetbook sandbox / webhook 후반부 상태 변화
