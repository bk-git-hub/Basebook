...
- Date: `2026-04-14`
- Time: `20:49`
- Agenda: Public sample image hosting path before full frontend-to-Railway cutover
- Participants: User, Codex
- Options Considered:
  - Keep sample images local under `apps/web/public` until the entire frontend and backend are moved to Railway
  - Move both the API and the sample-image path to deployed infrastructure immediately
  - Keep the API on local `localhost:4000` for now and move only reusable sample images to Cloudflare R2 first
- Decision:
  - Keep the current API integration path on local `localhost:4000` during this test phase
  - Move the reusable sample images to public Cloudflare R2 URLs first
  - Add a backend-owned batch upload script so the same sample-image set can be re-uploaded without manual one-by-one work
- Rationale:
  - Sweetbook-related image flows benefit from stable public URLs now, but moving the whole integration surface to Railway at the same time would increase debugging scope
  - The user wants a safer intermediate step that improves realism without forcing a full deployment switch before frontend stabilization
  - A backend-owned script keeps the public asset preparation reproducible and inside the backend maintenance boundary
- Impact:
  - `apps/api` now owns `npm run r2:upload-samples -w apps/api` for batch-uploading `apps/web/public/sample-images`
  - frontend can switch from local sample image paths to public R2 URLs without waiting for the full API deployment path
  - the API base URL remains local for now, so image hosting and API hosting can be validated independently
- Owner: User
- Status: `approved`
- Related Docs:
  - `apps/api/scripts/upload-sample-images.cjs`
  - `docs/AGENT_SYNC.md`
- Related Milestones:
  - `docs/milestones/backend.md`
